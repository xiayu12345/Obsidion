#!/bin/bash
# H133 p1_nor_ZS101（NOR；屏驱 zs101_ili9881c / ILI9881C，触摸 GT928）
# Usage: build_p1_nor_ZS101.sh [full|kernel|rootfs]
...
verify_jl_uboot_config() {
	local u="$ROOT/brandy/brandy-2.0/u-boot-2018"
	if ! grep -q '^CONFIG_LCD_SUPPORT_ZS101_ILI9881C=y' "$u/.config"; then
		echo "error: U-Boot LCD_SUPPORT_ZS101_ILI9881C is not enabled" >&2
		return 1
	fi
}
...
touch_lvgl_sunxifb_for_rebuild() {
	...
	[ -f "$f" ] && touch "$f"
	rm -rf "$ROOT/out/h133/p1_nor_ZS101/openwrt/build_dir/target/desk" 2>/dev/null || true
	rm -rf "$ROOT/out/h133/p1_nor_ZS101/openwrt/build_dir/target/service_guardian" 2>/dev/null || true
}

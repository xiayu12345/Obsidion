...
#ifdef GSL_NOID_VERSION
{
	...
	for (i = 0; i < (touches > MAX_FINGERS ? MAX_FINGERS : touches); i++) {
		...
		if (1 <= id && id <= MAX_CONTACTS) {
			...
			report_data(ts, y_new, x_new, 50, id);
			id_state_flag[id] = 1;
		}
	}
...
//	tp_node = of_find_node_by_name(NULL,"tp_1024");
	...
	else if(SCREEN_MAX_X==1366 && SCREEN_MAX_Y==768)
	{
		gsl_config_data_id = gsl_config_data_id_1366_768;
		GSLX680_FW =(struct fw_data *) GSLX680_FW_1366_768;
		id_len = ARRAY_SIZE(gsl_config_data_id_1366_768);
		fw_len = ARRAY_SIZE(GSLX680_FW_1366_768);
		printk("-------TP:1366_768\n");
	}
	printk("GSLX680 Enter %s\n", __func__);
	...
	of_property_read_u32_index(tp_node,"max-xy-flag",0,&ts->lcd_flag);
	printk("-------max-xy-flag==%d\n",ts->lcd_flag);

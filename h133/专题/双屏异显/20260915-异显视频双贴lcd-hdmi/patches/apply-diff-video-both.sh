#!/usr/bin/env bash
# 异显播片：video_screen 2→3（LCD+HDMI 都贴）
# 在 SDK 根目录执行，或任意路径执行本脚本。
#
# 用法:
#   apply-diff-video-both.sh           # 打补丁
#   apply-diff-video-both.sh --check   # dry-run
set -euo pipefail
...
PATCH="$SCRIPT_DIR/01-libuapi-diff-video-screen-3.patch"
if [[ ! -f "$PATCH" ]]; then
	echo "错误: 找不到 $PATCH" >&2
	exit 1
fi
...
echo "板端: 停播 → disp_mode diff → 再点播；get 时 video_screen=3"

#!/bin/bash
# H133 p1_emmc_JL_HXR101 (eMMC 16G；屏驱 HXR10106B-28 / ILI9881C)
# Usage: build_p1_emmc_JL_HXR101.sh [full|kernel|rootfs]

ROOT="${H133_SDK_ROOT:-$(cd "$(dirname "$0")/.." && pwd)}"
BOARD=p1_emmc_JL_HXR101
LUNCH=h133-p1_emmc_JL_HXR101-tina
...
ensure_board_config() {
	...
	./build.sh autoconfig -o openwrt -i h133 -b "$BOARD" -n default || return 1
}
...
touch_lvgl_sunxifb_for_rebuild() {
	...
	[ -f "$f" ] && touch "$f"
	rm -rf "$ROOT/out/h133/p1_emmc_JL_HXR101/openwrt/build_dir/target/desk" 2>/dev/null || true
	rm -rf "$ROOT/out/h133/p1_emmc_JL_HXR101/openwrt/build_dir/target/service_guardian" 2>/dev/null || true
}
...
IMG="$ROOT/out/h133_linux_${BOARD}_uart0.img"
if [ -f "$IMG" ]; then
	echo "==> OK: $IMG"
	ls -la "$IMG"
